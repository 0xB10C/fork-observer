...
// The mining feature (see the stratum jobs section at the bottom) shows which block
// pools are currently mining on top of. On by default. Declared up here because
// blocktree.js reads it while drawing, which can happen before the bottom of this
// file has run.
const MINING_ENABLED = true
// Which jobs feed to take the being-mined blocks from. Empty (no ?mining param, or
// ?mining=stratum.work) uses the default feed; ?mining=mpo switches to the second
// one (miningpool.observer), kept around to cross-check the first.
...
// A second feed carrying the same kind of jobs over a WebSocket, kept around so the
// two can be cross-checked against each other. Selected via ?mining=mpo; both feeds
// are consumed into the same state_stratum_jobs map, so nothing downstream of
// record_stratum_job() knows or cares which one is running.
...
// The two feeds differ only in how they spell a job, so each is boiled down here to
// the three fields drawn from: which pool, which block it builds on (display hash),
// and at which height. Everything else either feed sends on top of that - stratum.work's
// coinbase halves and extranonce sizes, miningpool.observer's coinbase_tag and
// coinbase_sum - isn't drawn, so it's dropped rather than carried around. Returns null
// for anything that isn't a job, which includes stratum.work's one-time
// {"type":"connection"} greeting.
const STRATUM_FEEDS = {
  "stratum.work": {
    url: STRATUM_SSE_URL,
...
// Takes an already-parsed job (see STRATUM_FEEDS[...].parse). Returns true when the
// job moves this pool onto a block no other pool was mining on, i.e. when it changes
// which blocks are drawn rather than just who is on them.
...
// it, and redraw if that changed which blocks are drawn.
...
// connect to the selected feed (default stratum.work); ?mining=mpo switches to the
// second one for cross-checking.
{
