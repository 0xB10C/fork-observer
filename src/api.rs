use crate::activity::{Activity, ActivityJsonResponse, ActivityQuery, RING_BUFFER_SIZE};
...
/// Default number of events an activity API request returns.
const DEFAULT_ACTIVITY_COUNT: usize = 50;
...
/// Serves recent activity log events of a network, newest first.
///
...
pub async fn activity_response(
    ...
    let count = query
        .count
        .unwrap_or(DEFAULT_ACTIVITY_COUNT)
        .min(RING_BUFFER_SIZE);
    let events = match activity {
        Some(activity) => match query.before {
            Some(before) => match activity.events_before(network_id, before, count).await {
                Ok(events) => events,
                Err(e) => {
                    error!(
            ...
            None => activity.recent_events(network_id, count).await,
        },
        None => vec![],
    };
...
mod tests {
    ...
    async fn activity_json_recent_and_pagination() {
        ...
        // Recent events (from the ring buffer), newest first, count respected.
        let resp = warp::test::request()
            .path("/api/0/activity.json?count=3")
            .reply(&route)
            .await;
        assert_eq!(resp.status(), 200);
        let v: serde_json::Value = serde_json::from_slice(resp.body()).unwrap();
        ...
        let oldest_id = events[2]["id"].as_i64().unwrap();

        // Paginate into the database with `before`.
        let resp = warp::test::request()
            .path(&format!(
                "/api/0/activity.json?count=3&before={}",
                oldest_id
            ))
            .reply(&route)
            .await;
